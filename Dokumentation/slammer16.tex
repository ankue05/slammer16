...
% In der PDF-Ausgabe
 \usepackage[colorlinks=true, bookmarks, bookmarksnumbered, bookmarksopen, bookmarksopenlevel=1,
  ...
  pdfauthor={Kürten, Schilling}, pdftex,
  pdftitle={SLAM - Simultaneous Localization and Mapping},
  pdfsubject={SLAM - Simultaneous Localization and Mappings},
  pdfkeywords={SLAM - Simultaneous Localization and Mapping}]{hyperref}
...
%In dieser Arbeit wird auf die Nomenklatur als Abkürzungs verzichtet. Bei Wunsch wieder aktivieren.
...
Wir konnten das Programm ohne Fehler kompilieren und haben uns zuerst die mitgelieferte Demo angeschaut. Wie oben beschrieben, wird bei dem Algorithmus eine Karte erstellt (siehe Abbildung \ref{dpslam_map1} und \ref{dpslam_map2}). Zur Erstellung der Karte kommt es nach einer bestimmten Anzahl von Iterationsschritten. Somit erhält man über die Zeit verschiedene Karten, die die abgescannte Gegend darstellen.

\begin{figure}[h]
	\includegraphics[scale=0.5]{images/hmap00.png}
	\label{dpslam_map1}
	\caption{Erste Karte nach erstem Durchlauf. Zu sehen ist ein Korridor.}
\end{figure}
\begin{figure}[h]
	\includegraphics[scale=0.2]{images/hmap162.png}
	\label{dpslam_map2}
	\caption{Karte nach mehreren Durchläufen. Zu sehen ist der Grundriss einer ganzen Etage.}
\end{figure}


Als nächsten Schritt haben wir versucht, das Programm mit Input-Daten von Dritten ans laufen zu bekommen. Dazu wurde der Quellcode etwas verändert um die Daten einlesen zu können. Als Input-Daten wurden die Daten vom \textit{tinySLAM}-Algorithmus genommen. Dieser Schritt hat erstaunlich gut geklappt. Als Output erschienen die zu erwarteten Karten, die mit \textit{tinySLAM}-Algorithmus übereinstimmten.\\
...
\begin{figure}[h]
	...
	\label{tinyLive}
\end{figure}
Ohne zusätzlich Informationen zu den verwendeten Testdaten war es uns nicht möglich diese zu deuten. Nichts desto trotz haben wir einfach einmal geschaut wie sich das Programm verhält wenn wir die Software mit anderen Datensätze (Abb. \ref{eigenesDatenformat}) ausführen.
Die Software lief im Gegensatz aller Erwartungen mit den andren Testdaten ohne Fehlermeldungen durch. Allerdings ohne Animation zur Laufzeit und fehlerhaften Endresultaten (Abb. \ref{tiny3} und \ref{tiny4}), was schon erahnen lässt, das die Testdaten nicht mit der Software kompatibel ist. Es scheint auf den Bilder so, als würde sich der Roboter nicht von der Stelle bewegen. Nach genauerem Vergleich mit den neuen Testdaten und den Daten welche bei tinySLAM an Bord waren, fiel uns auf das keine Informationen zur Odometrie und Ausrichtung des Roboters in den Daten vorhanden war. Das fehlen dieser Information, lässt darauf schließen dass diese Information aus den Daten zur Laufzeit berechnet werden. Da wir aber auch keine Information über die Ursprungsdaten haben, war es uns auch mit Hilfe des Quellcodes nicht möglich die Berechnung dieser Information nachzuvollziehen, und entschieden uns diesen Ansatz aufzugeben, da Einarbeitungszeit und mögliche Resultate nicht einzuschätzen waren.
...
\begin{figure}[h]
	\includegraphics[scale=0.15]{images/exp2_loop0_final.png}
	\caption{Erste Karte nach erstem Durchlauf des Roboters (neue Daten)}
	\label{tiny3}
\end{figure}
\begin{figure}[h]
	\includegraphics[scale=0.15]{images/exp2_loop1_final.png}
	\caption{Komplette Karte nach zweitem Durchlauf des Roboters (neue Daten)}
	\label{tiny4}
\end{figure}
...
Das Grundlegende Problem dabei war, die Entscheidung zu treffen wie lange man sich mit dem Problem und deren Lösung beschäftigt, obgleich man sich keineswegs sicher sein kann, das dass Resultat wirklich für unser Vorhaben geeignet ist.
 ...
 \textbf{Grund des Ausschlusses:}\\
 Nicht kompilierbar. Abhängigkeit zur alten Paketquellen, die nicht mehr existieren.
 
\subsection{FALKOlib}
FALKOlib (Fast Adaptive Laser Keypoint Orientation-invariant) ist eine Library, in der zwei neue \textit{Keypoint-Detectors} implementiert wurden.\\
Später stellte sich zudem herraus dass die Software "lediglich" interessante Schlüsselpunkte herrausfindet. Eine lösung für SLAM-Problem ist nicht vorgesehen, es dient nur zur Erweiterung eines Roboters der für ein SLAM-Problem eingesetzt wird.\\

\textbf{Grund des Ausschlusses:}\\
Keine Lösung des gewünschten Problems, zudem nicht kompilierbar.

\subsection{SLoM - Sparse Least Squares on Manifolds}
Das SLoM - Framework wollten wir genauer untersuchen, da damit geworben wurde das verschiedene Datenformate(LIDAR,Bilder etc.), und damit vielleicht auch unterschiedliche Ausführungen von Entfernungsdaten verarbeitet werden können. Der Algorithmus löst das SLAM-Problem mittels \textit{Least Squares on Manifolds} - Methode.\\

\textbf{Grund des Ausschlusses:}\\
Nicht kompilierbar.

\subsection{RFS-SLAM}
RFS-SLAM ist ebenfalls ein komplettes Framework wo man eigene Modelle zu speziellen Problemen definieren kann. RFS-SLAM löst das SLAM-Problem mittels \textit{Random Finite Set} - Estimation.
\\

\textbf{Grund des Ausschlusses:}\\
Laufzeitfehler.

\subsection{Linear SLAM}
