...
int ReadLog(FILE *logFile, TSense &sense, int &continueSlam) {
  ...
  if (FILE_FORMAT == REC) {
    ...
    ReadExternalLog(logFile, sense, continueSlam, line);

  }
  return 0;
}

int ReadExternalLog(FILE *logFile, TSense &sense, int &continueSlam,  char* line) {
  int i, max;
  if (!strncmp(line, "FLASER", 6)) {
    //fprintf(stderr, "Read line - done 1 ");
    strtok(line, " ");
    max = (int) (atof(strtok(NULL, " ")));
    if (max > SENSE_NUMBER)
      max = SENSE_NUMBER;
    ...
    //fprintf(stderr, "Read line - done 2 ");
    //ODEMETRY
    ...
    odometry.y = atof(strtok(NULL, " "));
    odometry.theta = atof(strtok(NULL, " "));
    //fprintf(stderr, "Read line x: %d, y: %d , theta: %d \n", odometry.x, odometry.y, odometry.theta);
    if (odometry.theta > M_PI)
      odometry.theta = odometry.theta - 2 * M_PI;
    else if (odometry.theta < -M_PI)
      odometry.theta = odometry.theta + 2 * M_PI;
